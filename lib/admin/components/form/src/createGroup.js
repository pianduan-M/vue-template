...
function createGroupTitle(groupItem) {
  ...
  return h('div', { style: titleStyles, class: titleClass }, h('span', {}, title));
}
...
function createFormItemsContent(props, groupItem) {
  ...
  return h('div', { style: contentStyle, class: contentClass }, createFormItems.call(this, props));
}

// 创建分割线
function createGroupSplitLine(groupItem) {
  const { lineStyle = {}, lineClass = [] } = groupItem;
  return h('div', { style: lineStyle, class: ['pd-form__line', ...lineClass] });
}
